...
async function addEmployee(req, res) {
    try {
        const { empId, empName, empEmail, empPassword, empPhno, taskCount, empOrgId, empServiceId } = req.body;
...
async function getEmployee(req, res) {
    try {
        ...
        console.error(err);
        res.status(500).json("Error in finding employee with Id " + empId);
    }
}
...
async function deleteEmployee(req, res) {
    try {
        ...
        if (result.deletedCount === 0) {
            ...
            const orgResult = await Organization.updateOne(
                { orgId : emp.empOrgId },
                { $pull: { empIds: empId } }
            );
    
            if (orgResult.nModified === 0) {
                return res.status(404).send(`Organization with ID: ${orgId} not found or Employee ID not associated with this Organization`);
            }
...
module.exports = {
    addEmployee,
    getEmployees, deleteEmployee, editEmployee, getEmployee
};